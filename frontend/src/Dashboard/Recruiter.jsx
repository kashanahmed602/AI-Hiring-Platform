...
const RecruiterDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900">
      ...
      <main className="lg:ml-64 relative z-10">
        <header className="h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-5 sm:px-8 flex items-center justify-between sticky top-0 z-20">
          ...
          <div className="hidden lg:block">
            <p className="text-xs text-slate-400">Monday, September 7</p>
            <h2 className="font-semibold">Hiring Workspace</h2>
          </div>
            ...
            <div className="xl:col-span-2 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Ranked by AI</p>
                </div>
                ...
                {candidates.map((candidate) => (
                  <div key={candidate.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-400 truncate">{candidate.role}</p>
                    </div>

                    <div className="text-right">
                      <p className="text-sm font-bold text-violet-600">{candidate.score}%</p>
      ...
      {jobModal && <JobModal onClose={() => setJobModal(false)} />}
    </div>
  );
};
